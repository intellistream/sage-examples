...
class MunicipalDocSource(ListBatchSource):
    ...
    def load_items(self) -> list[dict[str, Any]]:
        ...
        for item in items:
            item.setdefault("app_slug", "municipal_search")
            item["question_text"] = question_text
            item.setdefault("source_path", self.doc_dir)
        return items
...
class MunicipalQuestionMatcher(MapFunction):
    def execute(self, item: dict[str, Any]) -> dict[str, Any]:
        payload = dict(item)
        overlap = _tokenize(str(payload.get("question_text") or "")) & _tokenize(
            str(payload.get("municipal_chunk") or "")
        )
...
class MunicipalAnswerFormatter(MapFunction):
    def execute(self, item: dict[str, Any]) -> dict[str, Any]:
        payload = dict(item)
        source_name = Path(str(payload.get("source_path") or "")).name or "document"
        payload["answer_excerpt"] = str(payload.get("municipal_chunk") or "")[:220]
        payload["citation"] = f"{source_name}#chunk-{payload.get('chunk_index', 0)}"
        payload["answer_summary"] = (
            f"Municipal clause from {source_name}, citation {payload.get('citation')}, "
