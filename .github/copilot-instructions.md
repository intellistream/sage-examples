...
- Read docs first: `README.md`, `tutorials/README.md`, `tutorials/QUICK_START.md`,
  `examples/README.md`.
...
- Do not rely on manual install commands for permanent deps; declare in
...
- If work depends on SAGE core behavior, ensure SAGE dev env is prepared via
  `./quickstart.sh --dev --yes` in `SAGE/`.
- Keep dependency changes explicit and versioned in this repo files.

## Workflow

1. Confirm target area (`examples/`, `tutorials/`, or `sage-apps/`).
1. Implement minimal repo-local change.
1. Run relevant tests/linters and update nearby README when behavior changes.
...
- For cross-repo rollout, publish this repo/package first, then bump the version pin in
  `SAGE/packages/sage/pyproject.toml` when applicable.
